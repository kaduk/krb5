    ...
    krb5_context context;
{
     ...
     */
    retval = krb5_cc_resolve(context, tmpcc, &ccache);
    if (retval) {
        com_err(progname, retval, _("while opening credential cache %s"), tmpcc);
        exit(1);
    }

    retval = krb5_cc_initialize(context, ccache, my_principal);
    if (retval) {
        com_err(progname, retval, _("when initializing cache %s"), tmpcc);
        exit(1);
    }
