 ...
 *
 * Algorithm: if princname is not found, return false.
 *            if it is found and cmd is NULL, then the file entry after the
 *            principal name must be nothing or '*';
 *            if cmd is not NULL, then the entry must be matched ('*' is
 *            still okay).
 */
krb5_error_code
k5users_lookup(FILE *fp, char *princname, char *cmd, krb5_boolean *found,
               char **out_fcmd)
{
    krb5_error_code retval;
    char *line;
        ...
        if (fprinc && (!strcmp(princname, fprinc))) {
            ...
            if (cmd == NULL) {
                ...
                if (fcmd != NULL) {
                    ...
                    krb5_boolean match;
                    do {
                        if (match_commands(fcmd, cmd, &match, &temp_rfcmd,
                                           &err)) {
                            if (auth_debug) {
...
get_line(FILE *fp, char **out_line)
{
    ...
    line = xcalloc(BUFSIZ, sizeof (char ));
    line_ptr = line;
    line[0] = '\0';
