 ...
 * @param[in] context
 * @param[in] cc_def
 * @param[in] cc_other_tag
 * @param[in] primary_principal
 * @param[in] target_uid
 * @param[out] cc_out
 * @param[out] stored
*/
krb5_error_code
krb5_ccache_copy(krb5_context context, krb5_ccache cc_def, char *cc_other_tag,
                 krb5_principal primary_principal, krb5_ccache *cc_out,
                 krb5_boolean *stored, uid_t target_uid)
{
    int i = 0;
    ...
    cc_other = xcalloc(1, sizeof (krb5_ccache));

    retval = krb5_cc_resolve(context, cc_other_tag, cc_other);
    if (retval) {
    ...
    if (!stat(cc_def_name, &st_temp)) {
        retval = get_nonexp_tkts(context, cc_def, &cc_def_creds_arr);
        if (retval)
            return retval;
    }
    ...
    *stored = find_princ_in_cred_list(context, cc_def_creds_arr,
    ...
    if (cc_def_creds_arr != NULL) {
        while (cc_def_creds_arr[i] != NULL) {
                ...
                krb5_creds **creds_other)
{
    ...
    if (temp_creds != NULL) {
        while (temp_creds[i] != NULL) {
            retval= krb5_cc_store_cred(context, cc, temp_creds[i]);
            if (retval)
                return retval;
            i++;
        }
                ...
                krb5_creds ***creds_array)
{
    ...
    temp_creds = malloc(CHUNK * sizeof(krb5_creds *));
    if (temp_creds == NULL)
        return ENOMEM;
    ...
    retval = krb5_cc_next_cred(context, cc, &cur, &creds);
    while (!retval) {
        ...
        if (retval) {
            ...
            if (count == chunk_count * CHUNK - 1) {
                chunk_count++;
                temp_creds = realloc(temp_creds, chunk_count * CHUNK *
                                     sizeof(krb5_creds *));
                if (temp_creds == NULL)
                    return ENOMEM;
            }
...
printtime(time_t tv)
{
    ...
    tstamp = tv;
    fill = ' ';
    if (!krb5_timestamp_to_sfstring(tstamp, fmtbuf, sizeof(fmtbuf), &fill)) {
        printf("%s", fmtbuf);
    }
}
                      ...
                      krb5_principal primary_principal)
{
    ...
    if (!stat(ccs_name, &st_temp)) {
        retval = get_nonexp_tkts(context,  ccs, &ccs_creds_arr);
        if (retval)
            return retval;
    }
                      ...
                      krb5_principal prst, krb5_boolean *stored)
{
    ...
    if (temp_creds != NULL) {
        while (temp_creds[i] != NULL) {
            if (krb5_principal_compare(context, temp_creds[i]->client,
                                       prst) == TRUE) {
                retval = krb5_cc_store_cred(context, cc, temp_creds[i]);
                if (retval)
                    return retval;
 ...
 */
krb5_error_code
krb5_ccache_copy_restricted(krb5_context context, krb5_ccache cc_def,
                            char *cc_other_tag, krb5_principal prst,
                            ...
                            uid_t target_uid)
{
    ...
    retval = store_some_creds(context, *cc_other, cc_def_creds_arr,
                              cc_other_creds_arr, prst, stored);
    ...
    i=0;
    if (cc_other_creds_arr != NULL) {
        while (cc_other_creds_arr[i] != NULL) {
            krb5_free_creds(context, cc_other_creds_arr[i]);
            i++;
        }
    }
...
krb5_ccache_filter(krb5_context context, krb5_ccache cc, krb5_principal prst)
{
    ...
    if (!stat(cc_name, &st_temp)) {
        if (auth_debug) {
            fprintf(stderr,
                    "putting cache %s through a filter for -z option\n",
                    cc_name);
        }
        retval = get_nonexp_tkts(context, cc, &cc_creds_arr);
        if (retval)
            return retval;
        ...
        if (retval)
            return retval;
        retval = store_some_creds(context, cc, cc_creds_arr, NULL, prst,
                                  &stored);
        if (retval)
            return retval;

        if (cc_creds_arr != NULL) {
            while (cc_creds_arr[i] != NULL){
                        ...
                        krb5_principal princ)
{
    ...
    if (creds_list != NULL) {
        while(creds_list[i] != NULL) {
            if (krb5_principal_compare(context, creds_list[i]->client,
                                       princ) == TRUE) {
                temp_stored = TRUE;
                break;
            }
